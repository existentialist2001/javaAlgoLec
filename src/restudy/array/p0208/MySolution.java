...
복습
복습 할 때 새로 풀어서 맞췄는데, 처음 푼 풀이 보니까 가관이다.
빙빙 돌아가서 어렵게 풀고있다..
복습하면서 알게 된 것은, 처음 풀이처럼 내림차순을 만들기 위해 오름차순으로 만들고, 뒤집을 필요 없이
함수에 비교자(기준)를 넣어주면 된다는 것이다.

강의 풀이는 나보다 큰 점수만 카운팅해주면서 중복 점수문제까지 한번에 해결했다.
*/
